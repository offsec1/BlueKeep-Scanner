...
def scan(target_ip, verbose):
    ...
    try:
        ...
        if res:
            log("Target: {} VULNERABLE".format(target_ip))
        else:
            log("Target: {} NOT VULNERABLE".format(target_ip))
    except:
        error("Error executing scan -> Target: {} NOT VULNERABLE".format(target_ip))
...
class RDP:
    ...
    def rdp_scan_vuln(self):
        ...
        res = self.rdp_send_recv(self.rdp_encrypted_pkt(self.rdp_client_info(), rc4_ctx, hmackey, b"\x48\x00"))

        # recv server demand active packet
        res = self.rdp_recv()

        # send (encrypted) client confirm active PDU
        self.rdp_send(self.rdp_encrypted_pkt(self.rdp_client_confirm_active(), rc4_ctx, hmackey, b"\x38\x00"))

        # send (encrypted) synchronize PDU
        self.rdp_send(
            self.rdp_encrypted_pkt(binascii.unhexlify("16001700f103ea030100000108001f0000000100ea03"), rc4_ctx,
                                   hmackey))

        # send (encrypted) client control cooperate PDU
        self.rdp_send(
            self.rdp_encrypted_pkt(binascii.unhexlify("1a001700f103ea03010000010c00140000000400000000000000"), rc4_ctx,
                                   hmackey))

        # send (encrypted) client control request control PDU
        self.rdp_send(
            self.rdp_encrypted_pkt(binascii.unhexlify("1a001700f103ea03010000010c00140000000100000000000000"), rc4_ctx,
                                   hmackey))

        # send (encrypted) persistent key list PDU
        self.rdp_send(self.rdp_encrypted_pkt(self.rdp_client_persistent_key_list(), rc4_ctx, hmackey))

        # send (encrypted) font list PDU
        self.rdp_send(
            self.rdp_encrypted_pkt(binascii.unhexlify("1a001700f103ea03010000010c00270000000000000003003200"), rc4_ctx,
                                   hmackey))

        # session now fully established; attempt to unbind
        return self.rdp_try_unbind(rc4_ctx, hmackey)
    ...
    def rdp_encrypted_pkt(self, data, rc4_enc_key, hmac_key, flags=b"\x08\x00", flags_hi=b"\x00\x00",
                          channel_id=b"\x03\xeb"):
